...
void AlphaTrie(FILE * fAT){
    ...
    // Trie du Tableau buffer en fonction des noms -- TEST OK --
    while(flag==-1) // jusqu'à ce qu'il n'y ai plus de modifications effectuée dans la boucle
    {
        flag = 0;
        i = 0; j = 0;
            while(i < (nbF-1) && j <= min(strlen(Tbuffer[i].nom), strlen(Tbuffer[i+1].nom))) // Boucle de trie
            {
                if (Tbuffer[i].nom[j]>Tbuffer[i+1].nom[j] && Tbuffer[i+1].nom[j]!=0) // remet dans l'ordre alpha
                {
                    buffer = Tbuffer[i];
                ...
                else if (Tbuffer[i].nom[j]<Tbuffer[i+1].nom[j] && Tbuffer[i].nom[j]!=0) // laisse dans cet ordre
                {
                    j = 0, i++;
                }
                else // pass
    ...
    // Trie du Tableau buffer en fonction des prénoms en cas de nom identiques -- TEST OK --
    do
    {
        flag = 0;
        i = 0; j = 0; k = 0;
        // Test d'identité du nom pour toutes les fiches du tableau
        for (size_t i = 0; i < nbF-1; i++)
        {
            for (size_t j = i+1; j < nbF-1; j++)
            {
                if (strcmp(Tbuffer[i].nom, Tbuffer[j].nom)==0 && i!=j) // Si identité alors si désordre remet dans l'ordre, si égalité regarde le caractère suivant sinon ne fait rien
                {
                    if(Tbuffer[i].prenom[k]>Tbuffer[j].prenom[k] && Tbuffer[i].prenom[k]!=0 && Tbuffer[j].prenom[k]!=0  && Tbuffer[i].prenom[k]!=1 && Tbuffer[j].prenom[k]!=1) // Si dans le désordre
                    {
                        buffer = Tbuffer[i];
                        Tbuffer[i] = Tbuffer[j];
                        Tbuffer[j] = buffer;
                    ...
                    else if(Tbuffer[i].prenom[k]==Tbuffer[j].prenom[k] && Tbuffer[i].prenom[k]!=0 && Tbuffer[j].prenom[k]!=0 && Tbuffer[i].prenom[k]!=1 && Tbuffer[j].prenom[k]!=1) // Si égalité
                    {
                        k++;
                    }
                }
    ...
    // On retrie par nom parce que ça me fait encore du yolo...
    // Trie du Tableau buffer en fonction des noms -- TEST OK --
    while(flag==-1) // jusqu'à ce qu'il n'y ai plus de modifications effectuée dans la boucle
    {
        flag = 0;
